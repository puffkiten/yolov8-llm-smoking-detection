    ...
    <el-dialog v-model="visible" width="780px" destroy-on-close :show-close="false" class="model-config-dialog">
      ...
      <template #footer>
        <div class="dialog-footer product-footer">
          <div class="footer-right-actions footer-actions-group">
            <button class="dialog-action-btn secondary-btn native-action-btn dialog-action-button" type="button" :disabled="!canTestDialog || dialogTesting" @click="testDialog">{{ dialogTesting ? '测试中...' : '测试连接' }}</button>
            <button class="dialog-action-btn secondary-btn native-action-btn dialog-action-button" type="button" @click="visible = false">取消</button>
            <button class="dialog-action-btn primary-btn native-action-btn dialog-action-button" type="button" :disabled="!canSaveDialog || saving" @click="submit">{{ saving ? '保存中...' : '保存配置' }}</button>
          </div>
        </div>
